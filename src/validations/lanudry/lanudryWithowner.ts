...
export const lanudryWithownerValidations = [
  // Email validation
  body('laundryName')
  .notEmpty().withMessage('اسم المغسلة مطلوب') // Username is required
  .isLength({ min: 3 }).withMessage('يجب أن يكون اسم المغسلة على الأقل 3 أحرف') // Username must be at least 3 characters
  .custom(async (value: any) => {
    const user = await prisma.laundry.findFirst({ where: { name: value } });
    if (user) {
      throw new BadRequest('اسم المغسلة مستخدم بالفعل'); // Username already in use
    }
  })
  .isString().withMessage('اسم المغسلة يجب ان يكون نص'), // Name must be a string
  body('email')
